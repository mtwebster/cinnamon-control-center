...
struct _CsdUdevDeviceManagerClass
{
	CsdDeviceManagerClass parent_class;
};

G_DEFINE_TYPE (CsdUdevDeviceManager, csd_udev_device_manager, CSD_TYPE_DEVICE_MANAGER)
...
static void
add_device (CsdUdevDeviceManager *manager,
...
static void
remove_device (CsdUdevDeviceManager *manager,
	       GUdevDevice	    *udev_device)
{
	CsdDevice *device;
...
static void
csd_udev_device_manager_init (CsdUdevDeviceManager *manager)
{
...
static void
csd_udev_device_manager_finalize (GObject *object)
{
	CsdUdevDeviceManager *manager = CSD_UDEV_DEVICE_MANAGER (object);

	g_hash_table_destroy (manager->devices);
	g_object_unref (manager->udev_client);

	G_OBJECT_CLASS (csd_udev_device_manager_parent_class)->finalize (object);
}

static GList *
csd_udev_device_manager_list_devices (CsdDeviceManager *manager,
				      CsdDeviceType	type)
{
	CsdUdevDeviceManager *manager_udev = CSD_UDEV_DEVICE_MANAGER (manager);
...
static CsdDevice *
csd_udev_device_manager_lookup_device (CsdDeviceManager *manager,
				       GdkDevice	*gdk_device)
{
	...
	g_hash_table_iter_init (&iter, CSD_UDEV_DEVICE_MANAGER (manager)->devices);
...
static void
csd_udev_device_manager_class_init (CsdUdevDeviceManagerClass *klass)
{
	CsdDeviceManagerClass *manager_class = CSD_DEVICE_MANAGER_CLASS (klass);
	GObjectClass *object_class = G_OBJECT_CLASS (klass);

	object_class->finalize = csd_udev_device_manager_finalize;
	manager_class->list_devices = csd_udev_device_manager_list_devices;
	manager_class->lookup_device = csd_udev_device_manager_lookup_device;
}
