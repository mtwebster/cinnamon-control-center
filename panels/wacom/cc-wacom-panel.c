		     ...
		     GdkDeviceTool *tool)
{
	...
	wacom_device = g_hash_table_lookup (priv->devices, csd_device);
	if (!wacom_device)
		return;
...
static gboolean
on_toplevel_event_cb (GtkWidget    *wigdet,
		              GdkEvent     *event,
		              CcWacomPanel *panel)
{
...
static void
on_panel_realized (GtkWidget *widget,
                   GtkWidget *old_parent,
                   gpointer   user_data)
{
    CcWacomPanel *panel = CC_WACOM_PANEL (widget);
    GtkWidget *toplevel = gtk_widget_get_toplevel (widget);

    g_signal_connect_object (toplevel, "event",
                             G_CALLBACK (on_toplevel_event_cb),
                             panel, 0);
}
...
cc_wacom_panel_constructed (GObject *object)
{
    ...
    g_signal_connect (object, "parent-set", G_CALLBACK (on_panel_realized), NULL);
}
		  ...
		  CsdDevice    *csd_device)
{
	...
	g_hash_table_insert (priv->devices, csd_device, device);
		   ...
		   CcWacomPanel     *self)
{
	...
	device = g_hash_table_lookup (self->priv->devices, csd_device);
	if (!device)
		return;

	g_hash_table_steal (self->priv->devices, csd_device);
				                ...
				                const char   *panel)
{
    ...
    cmd = g_strdup_printf ("cinnamon settings %s", panel);
...
cc_wacom_panel_init (CcWacomPanel *self)
{
	...
	priv->manager = csd_device_manager_get ();
	...
	devices = csd_device_manager_list_devices (priv->manager,
...
GDBusProxy *
cc_wacom_panel_get_csd_wacom_bus_proxy (CcWacomPanel *self)
{
