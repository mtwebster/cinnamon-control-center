...
cc_wacom_page_get_gdk_device (CcWacomPage *page)
{
	...
	csd_device = cc_wacom_device_get_device (page->priv->stylus);
...
set_osd_visibility (CcWacomPage *page)
{
	...
	priv = page->priv;
	proxy = cc_wacom_panel_get_csd_wacom_bus_proxy (priv->panel);
	csd_device = cc_wacom_device_get_device (priv->pad);

	device_path = csd_device_get_device_file (csd_device);
